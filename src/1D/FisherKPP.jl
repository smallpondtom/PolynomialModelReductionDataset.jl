...
export FisherKPPModel, build_fast_solver, integrate_model_fast
...
# ============================================================================
# Fast SICN / CNAB integrator — Crank-Nicolson on linear A, explicit / AB-2
# on quadratic F u². See `FastSolvers` for the underlying primitives.
# ============================================================================

"""
$(SIGNATURES)

Build a fast Crank-Nicolson-flavoured solver for FisherKPP (bakes `α = Δt/2`).
"""
function build_fast_solver(model::FisherKPPModel, params::Dict;
                            scheme::Symbol=:CN, Δt::Real=model.Δt)
    @assert scheme === :CN "FisherKPP fast integrators use Crank-Nicolson (scheme=:CN)"
    α = Float64(Δt) / 2
...
Fast SICN or CNAB integrator for FisherKPP.
...
- `u2_jm1=nothing`: seed for AB-2 (CNAB only)
"""
function integrate_model_fast(model::FisherKPPModel, solver::AbstractFastSolver,
    ...
    @inbounds for j in 2:Tdim
        ...
        if integrator_type === :SICN || (j == 2 && u2_jm1 === nothing)
            mul!(tmp, F, u2)
            @. rhs = rhs + Δt * tmp
        else
            mul!(tmp, F, u2)
